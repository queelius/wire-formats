...
Posts 1 through 10 of this series focused on encoding integers and sequences: universal codes, arithmetic coding, Huffman, LZ77. All of them turn a sequence into a compact bit string. This post shifts direction slightly. Instead of asking "how do we write bits compactly?" it asks "once we have a bit vector, how do we query it efficiently without expanding it?"
...
These appear everywhere: inverted indexes, compressed graphs, FM-indexes, wavelet trees. Rank tells you how many elements come before a position; select inverts that operation.
...
The succinct approach hits the sweet spot: it adds an auxiliary index that is asymptotically negligible (sublinear, hence $o(n)$) while buying constant-time rank. Select is $O(\log n)$ with a binary search; the $O(1)$ extension requires a separate "select samples" structure and is covered in a footnote.
...
All three steps are $O(1)$: two array indexing operations and one hardware-accelerated popcount (POPCNT on x86-64, VCNT on ARM). The key insight is that we precompute partial sums at two granularities. The superblock array handles the large-scale prefix sum; the block array handles the within-superblock refinement. The final popcount handles the within-word residual.

## Select via Binary Search

Select is harder than rank because the answer is nonlocal: the $j$-th 1-bit could be anywhere, and we cannot jump directly to it without additional precomputed information.
...
The $O(1)$ select extension stores one additional array: a "select samples" array holding the position of every $(\log^2 n)$-th 1-bit. That halves the binary-search range and allows $O(1)$ select at the cost of a few extra kilobytes for typical $n$. PFC's production version implements this. For the pedagogical version here, $O(\log n)$ select is sufficient to demonstrate the concept.

## Where Succinct Bit Vectors Show Up

Inverted indexes in full-text search use bit vectors to represent posting lists (which documents contain a term). Rank and select over those bit vectors power gap compression: the posting list stores gaps between document IDs, and select gives back the original IDs.
...
In all of these, the n + o(n) space guarantee matters: the index must not dwarf the data it queries.
