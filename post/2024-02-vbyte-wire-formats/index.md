...
**Production code:** The `VByte` struct in this post matches the algorithmic structure of the production implementation in [PFC](https://github.com/queelius/pfc): see `include/pfc/codecs.hpp`. The PFC version integrates with `BitReader`/`BitWriter` infrastructure from `core.hpp` in the same bit-level style used here. For the SIMD-optimized byte-level version used in production databases, see Google's protobuf-cpp or Daniel Lemire's `FastPFOR` and `streamvbyte` libraries.
